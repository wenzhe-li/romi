#!/bin/bash
cd ..
mkdir outlogs_mopo_model &> /dev/null
declare -a tasks=(
...
for seed in "${seeds[@]}"
do
CUDA_VISIBLE_DEVICES=${n} nohup python scripts/train_model.py \
--env_name=${task} --seed=${seed} --train_reverse_model \
>& outlogs_mopo_model/${task}_${algo}_${seed}.log &
