#!/bin/bash
cd ..
mkdir outlogs_romi_bcq &> /dev/null
declare -a tasks=(
...
for seed in "${seeds[@]}"
do
CUDA_VISIBLE_DEVICES=${n} nohup python scripts/train_bcq.py \
--env_name=${task} --seed=${seed} --train_bcq --bcq_policy_dir=romi_bcq_${task}_models \
>& outlogs_romi_bcq/${task}_${algo}_${seed}.log &
