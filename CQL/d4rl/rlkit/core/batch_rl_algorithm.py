...
class BatchRLAlgorithm(BaseRLAlgorithm, metaclass=abc.ABCMeta):
    ...
    def buffer_sample(self, batch_size):
        ...
        for item in zip(*data):
            sub_data = torch.cat(item, 0)
            output.append(sub_data)
        state, action, next_state, reward, weight, not_done = output
        batch = dict(
            observations=state.numpy(),
            actions=action.numpy(),
            rewards=reward.numpy(),
            terminals=1. - not_done.numpy(),
            next_observations=next_state.numpy(),
        )
        return batch
            ...
            for _ in range(self.num_train_loops_per_epoch):
                ...
                for _ in range(self.num_trains_per_train_loop):
                    if self.buffer is None:
                        ...
                        train_data = self.buffer_sample(self.batch_size)
                        # print(train_data)
                        # assert False
